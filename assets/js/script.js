...
function getHotelToken () {
    ...
    .then(function (response) {
      ...
      .then(function () {
        let requestUrl = "https://test.api.amadeus.com/v2/shopping/flight-offers?originLocationCode=PHL&destinationLocationCode=ORD&departureDate=2023-11-15&returnDate=2023-11-17&adults=1&nonStop=true&currencyCode=USD&max=25"
        fetch(requestUrl, {
            method: "GET",
            headers: {'Authorization': 'Bearer ' + token}
        })
        ...
        .then(function(data){
            console.log(data)
            const flightOptions = $("#flightChoice")
            for(i = 0; i < data.data.length; ++i) {
                ...
                flightOptions.children().eq(i).append(departFlightTime);
                arrivalFlightTime.textContent = flights.arrivalTime[i];
                flightOptions.children().eq(i).append(arrivalFlightTime);
                returnDepartTime.textContent = flights.returnDepartTime[i];
                flightOptions.children().eq(i).append(returnDepartTime);
                returnArrivalTime.textContent = flights.returnArrivalTime[i];
                flightOptions.children().eq(i).append(returnArrivalTime);
                departAirlineCode.textContent = flights.departAirline[i];
                flightOptions.children().eq(i).append(departAirlineCode);
                ...
                flightOptions.children().eq(i).append(flightPrice);
